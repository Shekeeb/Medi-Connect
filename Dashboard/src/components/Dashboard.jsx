...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const { data } = await axios.get("https://medi-connect-backend.onrender.com/api/v1/appointment/getall",
  ...
  const handleUpdateStatus = async (appointmentId, status) => {
    try {
      const { data } = await axios.put(
        `https://medi-connect-backend.onrender.com/api/v1/appointment/update/${appointmentId}`,
        { status },
        { withCredentials: true }
      );
        ...
        <div className="banner">
          ...
          <div className="firstBox">
            ...
            <div className="content">
              <div>
                <p>Hello<h5> {admin && `${admin.firstName} ${admin.lastName}`}{" "}</h5></p>
              </div>
              <p>"I am thankful that you have confidence in me to help you".</p>
            </div>
          </div>
