...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const { data } = await axios.get("https://medi-connect-08yx.onrender.com/api/v1/appointment/getall",
  ...
  const handleUpdateStatus = async (appointmentId, status) => {
    try {
      const { data } = await axios.put(
        `https://medi-connect-08yx.onrender.com/api/v1/appointment/update/${appointmentId}`,
        { status },
        { withCredentials: true }
      );
